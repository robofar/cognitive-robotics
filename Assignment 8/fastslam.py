...
def probability(z, z_expected, measurement_covariance):
    measurement_covariance_inv = np.linalg.inv(measurement_covariance)
    z_diff = z - z_expected
    exponent = -0.5 * np.dot(np.dot(z_diff.T, measurement_covariance_inv), z_diff)
    prob = np.divide(1.0, (np.sqrt((2 * np.pi) ** len(z) * np.linalg.det(measurement_covariance)))) * np.exp(exponent)
...
class Particle(object):
    ...
    class LandmarkEKF(object):
        ...
        def __init__(self):
            self.observed = False
            self.mu = np.vstack([0, 0]).astype(float)  # landmark position as vector of length 2
    ...
    def correction_step(self, sensor_measurements):
        ...
        # Construct the sensor noise matrix Q (2 x 2) -> measurement noise
        Q_t = 0.1 * np.identity(2)

        # Particle "thinks" he is the correct robot pose
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                # TODO: Initialize its (i.e landmark) position based on the measurement and the current Particle pose:
                landmark.mu[0] = robot_pose[0] + (measurement.z_range * np.cos(robot_pose[2] + measurement.z_bearing))
                landmark.mu[1] = robot_pose[1] + (measurement.z_range * np.sin(robot_pose[2] + measurement.z_bearing))

                # get the Jacobian
                [h, H] = self.measurement_model(landmark)

                # TODO: initialize the EKF for this landmark
                landmark.sigma = np.matmul(np.linalg.inv(H), np.matmul(Q_t, np.transpose(np.linalg.inv(H))))
                ...
                measurement_covariance = np.matmul(H, np.matmul(landmark.sigma, np.transpose(H))) + Q_t

                # TODO: calculate the Kalman gain
                K = np.matmul(landmark.sigma, np.matmul(np.transpose(H), np.linalg.inv(measurement_covariance)))

                # TODO: compute the error between the z and expected_z (remember to normalize the angle)
                measurement = np.array([measurement.z_range, measurement.z_bearing])
                expected_measurement = np.array(expected_z).flatten()
                error_z = np.subtract(measurement, expected_measurement).reshape(-1,1)

                # TODO: update the mean and covariance of the EKF for this landmark
                landmark.mu = landmark.mu + np.matmul(K, error_z)
                landmark.sigma = np.matmul((np.eye(2) - np.matmul(K, H)), landmark.sigma)
    

                # TODO: compute the likelihood of this observation, multiply with the former weight
                # to account for observing several features in one time step
                p_measurement_given_landmark = probability(measurement, expected_measurement, measurement_covariance)
                self.weight = self.weight * p_measurement_given_landmark
...
class Plotter(object):
    ...
    def fast_slam(self, t):
        ...
        '''
        global num_iterations_resample
        ...
        if num_iterations_resample == None: 
    ...
    def plot_state(self, sensor_measurements):
        ...
        for landmark in self.particles[index_of_best_particle].landmarks:
            if landmark.observed:
                ...
                angle_degrees = normalize_angle(angle) * 180. / pi
